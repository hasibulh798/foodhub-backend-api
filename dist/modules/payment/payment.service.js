...
const initPayment = async (paymentData) => {
    try {
        const sslcz = new SSLCommerzPayment(process.env.STORE_ID, process.env.STORE_PASS, isLive);
...
export const handlePaymentFail = async (body) => {
    const { tran_id } = body;
    await prisma.order.update({
        where: { transactionId: tran_id },
